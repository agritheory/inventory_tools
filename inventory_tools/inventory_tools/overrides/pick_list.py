...
class InventoryToolsPickList(ERPNextPickList):
	def default_route_strategy(self) -> str | None:
		if not self.company:
			return None
	...
	def after_mapping(self, source_doc):  # noqa
		strategy = self.default_route_strategy()
...
class PathFinder:
	@staticmethod
	def query_stock_entries(item_code, qty, company, order_by_clauses, plan_warehouses, to_date):
	...
	def FIFO(item_code, qty, company, plan_warehouses=None, to_date=None):
		...
		return PathFinder.query_stock_entries(
