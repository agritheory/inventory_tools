...
def get_specification_items(attributes, start=0, sort_order=""):
	...
	def add_display_details(self, result, discount_list, cart_items):
		...
		for item in result:
			product_info = get_website_product_info(item.item_code, skip_quotation_creation=True).get(
				"product_info"
			)
...
def get_website_product_info(item_code, skip_quotation_creation=False):
...
def get_website_item_price(item_code, price_list, customer_group, company, qty=1, uom=None):
